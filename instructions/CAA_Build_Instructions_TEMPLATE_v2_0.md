...
36. A capacity-validated hub-bank envelope is a binding construction decision. Exact materialization must either (a) carry each leg's assigned bank ID forward as a hard timing-domain restriction or (b) reassign banks inside the same all-fleet optimization that enforces every arrival/departure row of the validated capacity envelope. Never reopen a leg to another bank in an independent or unconstrained downstream solve: that invalidates the upstream capacity proof and creates unnecessary symmetry/runtime. When joint reassignment is used, seed from the capacity-checked plan, retime both directions of an affected market together, and keep the flexible neighborhood no larger than required to clear the remaining overloaded rows. (Schedule 7 v0.1.0; joint-envelope extension from Milestone 0.7.11)
