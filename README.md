...
Milestone 0.7.9 adds a strict, auditable network boundary to the deterministic planning, demand, bank, topology-repair, exact-materialization, and canonicalization pipeline. Python reconstructs the historical plan, processes the complete pinned 105-city demand data, reproduces all 100 non-hub qualifications, and applies the current percentile-tier service caps before frequency allocation. Aircraft quantities still come only from each schedule's build configuration; no fleet count is inferred from policy or silently added by repair.
...
The v4 planning pin removes 131 historical hub/focus markets outside the effective tier assignments, preserves historical point-to-point markets, and retains BHM in one allowed slot for 20 cities in the documented FLP, GCP, OZK, and TEX groups. A 1,430-leg ceiling prevents that reconciliation from silently expanding the accepted proposal. The bank search now proves directional use on the exact five-minute grid, and topology repair supports separate balanced components within a fleet instead of requiring obsolete markets to connect them.

`exact_materialization_plan.json` selects one five-minute time for all 1,430 legs, aligns all 1,126 hub-touching legs, integrates all 304 non-hub legs, and produces real aircraft cycles using 211/225 aircraft. Canonicalization turns those cycles into 33 lines, 211 operating-day routes, 470 directed pairings, and flights 1001–2430 while preserving all 1,232 bank touches and replacing legacy workbook provenance with generated-source fingerprints. The exact cycle cadence calculation now measures a single target-city overnight against the full circular cycle; three deterministic successor swaps clear the resulting cadence failures before canonicalization.

Full validation now passes structure, curfews, minimum turns, numbering, bank alignment, rolling target-city RON cadence, and every percentile-tier service envelope. The 45 tier-cap findings are eliminated. The proposal remains review-required because the exact solve has 236 same-pairing spacing findings and gate/stand findings now touch 55 stations (17 physical assignment conflicts, 495 passenger touches on stands, 22 stand-capacity findings, and 22 combined-capacity peaks). The network concentration made gate feasibility materially harder, so spacing and gate/stand construction remain the next deterministic repair stages; these findings do not authorize extra aircraft, gates, stands, or curfew waivers.
